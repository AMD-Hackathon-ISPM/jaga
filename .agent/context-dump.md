...
We started from genuine curiosity: _what can you build with an MI300X that you couldn't easily build otherwise?_ Verified specs:
...
We set it aside for an honest reason: it leaned close to existing knowledge tools, and we wanted to build something that felt more distinctly **ours** and more clearly **needed**. What carried forward was the _fully-local-on-AMD privacy_ pattern and a high bar for documentation (captured in `plan-template.md`).

---

## 8. Phase 7 — Refusing to Build Me-Too

We pressure-tested several "underserved" niches and kept finding them already crowded in 2026 — special-ed tools, vet scribes, permit review, synthetic focus groups, AI grading. Rather than squeeze into a packed lane, we drew a line: **we won't ship a me-too product.** If almost every "AI does X" space is occupied, then what makes something worth building isn't a virgin market — it's a real, under-served _need_, a genuine angle, and execution we can stand behind. That clarified the search: find a problem that actually matters to people, where our approach is meaningfully different.

---

## 9. Phase 8 — Pulled Toward Real-World Impact

The conversation kept gravitating to high-impact medical AI — projects that detect disease and help people, not just optimize a workflow. We were honest about prior art (drug-discovery and brain-imaging AI are deep, well-trodden fields) and that was fine: we weren't looking to be first, we were looking to build something that _helps_ and that we can actually execute. Health, with public datasets and clear human stakes, felt like the right kind of hard.
...
**The name: Jaga** — Indonesian for _"to watch over / to guard."_ It says the mission in one word: something that watches over your lungs and your community. We deliberately avoided "breath"-type names (they echo existing tools) in favor of something authentic to us and easy for anyone to say.
...
- **Claim corrections.** "1/3 of cases missed" → WHO 2025: ~10.7M estimated vs 8.3M notified (~2.4M / ~22% gap). "Incumbents need a radiologist" is false — WHO permits AI-CAD to _replace_ readers (6 products approved Jun 2025). "Cough on a phone" isn't a moat — Swaasa already does cough+clinical phone screening. A photographed X-ray ≠ a digital CXR.
...
| Decision                                                                                                                                | Reasoning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Commit serving weights in-repo (no external download step)**                                                                          | A clean clone must run the documented commands end to end on submission day. Every serving file is under GitHub's 100 MB hard limit (YAMNet 15 MB + class map, XGBoost ONNX 52 KB, DenseNet `best.pt` 83 MB, ~98 MB total), so the weights are tracked directly via targeted `.gitignore` negations. Rejected: Google Drive links (auth interstitials, quotas, judges may hit a broken link), Git LFS (public-repo bandwidth quota), GitHub Releases + fetch script (extra moving part with no payoff at this size). Archives like `.rar` are never committed — extracted files only. Training data and non-serving artifacts stay ignored. See the 2026-07-11 log entry.                                                                                       |
| **Serve Grad-CAM as an in-memory PNG data URL (no artifact hosting)**                                                                   | Chosen over artifact hosting (MinIO / signed URLs — the approach `backend-integration-map.md` had suggested) because the overlay is small, request-scoped, and must never retain patient CXR data; nothing is persisted to disk, matching Jaga's no-retention posture for this signal. See the 2026-07-12 log entry.                                                                                                                                                                                                                                                                 |
...
| **Use one guided cough recording of up to 90 seconds**                                                                                  | The frontend now captures several natural coughs in one session, allows an early stop or automatic stop, and makes retry discard the whole recording and restart from zero. Its proposed multipart contract sends one WebM `cough` file; the backend remains unchanged pending explicit alignment.                                                                                                                                                                                                                                                                                      |
| **Use a client-side energy heuristic for live cough detection** *(superseded 2026-07-12)*                                               | Superseded by the YAMNet cough-episode decision below: the adaptive RMS detector, its live/review counts, and the orange-pulse feedback were removed because loudness alone cannot distinguish a cough from any other loud sound. See the 2026-07-12 log entry.                                                                                                                                                                                                                                                                                                                          |
| **Use YAMNet cough episodes as the only displayed cough count**                                                                          | The client loudness heuristic, live/review counts, and illustrative focus figure are retired. YAMNet groups consecutive above-threshold overlapping windows into approximate episodes; the gateway exposes their count and highlights the strongest episode in Daffa's spectrogram. The artifact shows model focus, not causal reasoning or a clinical explanation. |
| **Distill results into result → next step → evidence → optional detail**                                                                | The standalone “What the model looked at” card repeated metadata and caveats. Gema and Prisma now keep the participant-facing result and confirmatory instruction primary and consolidate signal/model/contract/cohort/calibration/limitations into one collapsed “About this estimate” disclosure. Gema promotes the honest recording summary. Prisma gives the uploaded CXR and optional heatmap the full stage width as a large side-by-side evidence pair. Result-class pages widen at 840 px instead of 1024 px so tablet and browser-preview widths do not waste half the canvas. |
...
| **Name the two signals Gema (cough+clinical) and Prisma (CXR)**                                                                         | Distinct names reinforce that the signals are separate and never fused. Both stay inside the Jaga "watch over" brand: _Gema_ = "echo" (the acoustic core that listens), _Prisma_ = "prism" (the imaging signal).                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Make the frontend contract-first and provider-agnostic**                                                                              | Fixture/live adapters, runtime schemas, sanitized fixtures, and `contracts/openapi/jaga-v1.yaml` give backend owners an exact handoff for patient intake, cough audio, independent digital CXR, results, readiness, and scoped guidance. Frontend types, fixtures, and services now propose one WebM `cough` file while the machine-readable and backend contracts remain unchanged pending alignment. Fireworks/Featherless credentials and model selection remain behind the Go gateway; no patient form values are sent to the assistant.                                            |
...
| **Rename `backend/go` → `backend/backendHandlers`; `backend/python/{PrismaServer,PrismaTraining}` → `backend/modelServerandTraining/{PrismaServer,PrismaTraining}`; add `backend/modelServerandTraining/{GemmaServer,GemmaTraining}`** | The build split Gema's acoustic model services (Rust, ONNX via `ort`) and research notebook into their own tree alongside Prisma's, and renamed the Go and Python roots along the way. Discovered by direct repository inspection on 2026-07-11 — no log entry recorded the rename at the time it happened, so `project-architecture.md` §11/§14 and `implementation-plan.md` §13 carried the stale `backend/go`/`backend/python` paths until this pass. The Go module path (`jaga/backend/go`) was not updated to match and is a harmless cosmetic leftover. |
| **Ship Gema without the signed evidence gate (participant-grouped split, calibration, promotion thresholds)**                            | Not a deliberate decision recorded anywhere — the pipeline (YAMNet gate → XGBoost on WavLM + demographics) was built and wired end to end under deadline pressure, and the rigor `data-evaluation-plan.md` specifies was not retrofitted. Recorded here as an acknowledged gap rather than a silent one: risk bands are a fixed 0.33/0.66 threshold on the raw probability, not a fitted calibration artifact. See `project-architecture.md` §16.2 and `data-evaluation-plan.md` §10–§11. |
...
**Gema** — the `[MVP]` cough-plus-clinical research signal (Indonesian for "echo"). The acoustic core that listens to one guided recording containing several natural coughs plus supported clinical inputs. Never fused with Prisma.
...
**Single-recording protocol** — one guided recording of up to 90 seconds, discarded in full when the user records again.
