 ...
 * segment fills; the band is also named in words by the caller, so color is
 * reinforcement, never the sole signal. Lower carries no green / checkmark.
 */
export function RiskBandTrack({ band, probability }: { band: RiskBand; probability?: number }) {
  const t = useT();
